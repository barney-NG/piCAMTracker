...
# News
2019-12-01: Next error in the image. (I have to create a 0.6.3 in the next days...)
Evereybody who is using a shutdown button on GPIO 4 (pin 7) needs to add _,gpio_pin=4_ to the line _dtoverlay=gpio-shutdown_ in /boot/config.txt
```
sudo nano /boot.config.txt
```
Scroll down to the line _dtoverlay=gpio-shutdown_ (mostly at the end of the file). Change the line to:
```
dtoverlay=gpio-shutdown,gpio_pin=4
```
and save the change with Ctrl+X and then type "Y" confirm saving. After next reboot GPIO 4 is the pin to shutdown the Raspberry.

2019-12-01: [V0.6.2 image](https://drive.google.com/open?id=1E4yqIbiqcDQ0TDi-UMaiOwQmygAPVL0O) released today. During the last update I overwrote the access rights for the web server. This is corrected in this version. If you don't want to create a complete update you can correct the access rights with the following commands:
```
cd piCAMTracker/
chgrp www-data www www/FIFO  
```
2019-11-30: V0.6.1 image released today. You need to install this image because development now depends on the latest Raspbian Buster operating system. Simple updates (git pull) will not work any longer on the v0.4 and v0.5 based systems. This image is running on Raspberry 4 B, 3 B and 3 B Plus.
...
* you can download the V0.6.2 image [here](https://drive.google.com/open?id=1E4yqIbiqcDQ0TDi-UMaiOwQmygAPVL0O)
  pi-password: Olav01
